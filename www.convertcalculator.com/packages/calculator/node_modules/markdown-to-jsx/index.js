...
export function compiler(markdown, options) {
    ...
    function compile(input) {
        ...
        let jsx;
        if (arr.length > 1) {
            jsx = inline ? < span key = "outer" > {
                arr
            } < /span> : <div key="outer">{arr}</div > ;
        ...
        refImage: {
            ...
            react(node, output, state) {
                return ( <
                    ...
                    src = {
                        sanitizeUrl(refs[node.ref].target)
                    }
                    title = {
                        refs[node.ref].title
                    }
        ...
        jsx.props.children.push( <
            footer key = "footer" > {
                footnotes.map(function createFootnote(def) {
                    return ( <
                        div id = {
                            options.slugify(def.identifier)
                        }
                        key = {
                            def.identifier
                        } > {
