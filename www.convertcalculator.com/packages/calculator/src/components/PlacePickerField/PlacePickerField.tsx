...
const PlacePickerField = ({
  ...
  const handleFetchPlaceSuggestions = async () => {
    if (!dobouncedCurrentValue || dobouncedCurrentValue.length < 3) {
      ...
      try {
        const result = await onFetchPlaceSuggestions({
  ...
  const handleSelectOption = async (option) => {
    ...
    // FF: placesViaGeoapify:  Remove this if statement and remove `onFetchPlaceDetails` dependency
    if (placesViaGeoapify) {
      onChange(name, option);
    } else {
      const placeDetails = await onFetchPlaceDetails({
        placeId: option._id,
      });

      onChange(name, { ...option, placeDetails });
    }
  };
...
export default PlacePickerField;
